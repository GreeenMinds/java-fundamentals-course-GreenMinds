# Fundamentos de Java para Principiantes

## Resumen del Curso

Este curso introduce a estudiantes de secundaria a los fundamentos de la programación utilizando Java, incluyendo conceptos básicos de Programación Orientada a Objetos (POO).

**Duración total:** ~60 minutos

**Público objetivo:** Estudiantes de 12 a 17 años sin experiencia previa en programación.

**Prerrequisitos:** Ninguno.

**Herramientas necesarias:** Solo un navegador web (Chrome, Edge, Firefox o Safari).

**Repositorio de Código Fuente:**  
[https://github.com/GreeenMinds/java-fundamentals-course-GreenMinds](https://github.com/GreeenMinds/java-fundamentals-course-GreenMinds)


## Secuencia de Lecciones

### Lección 1: ¿Qué es Java y la Programación? (5 minutos)

**Descripción:** Aprende qué es la programación y por qué Java es un excelente lenguaje para comenzar.

**Video:**  
https://youtu.be/[VIDEO-1]

**Conceptos clave:**
- ¿Qué es programar?
- ¿Qué es Java?
- Aplicaciones de Java.

**Práctica:**  
https://replit.com/[LECCION-1]


### Lección 2: Variables y Tipos de Datos (9 minutos)

**Descripción:** Aprende a almacenar información usando variables.

**Video:**  
[https://www.youtube.com/watch?v=z9eDDk3ONBE](https://www.youtube.com/watch?v=z9eDDk3ONBE)

**Conceptos clave:**
- Variables.
- int.
- double.
- String.
- boolean.

**Práctica:**  
[https://onlinegdb.com/BGwNMV7ZHf](https://onlinegdb.com/BGwNMV7ZHf)


### Lección 3: Estructuras de Control: Decisiones y Bucles (10 minutos)

**Descripción:** Aprende a tomar decisiones y repetir tareas.

**Video:**  
https://youtu.be/[VIDEO-3]

**Conceptos clave:**
- if.
- else.
- for.
- while.

**Práctica:**  
https://replit.com/[LECCION-3]


### Lección 4: Métodos, Entrada/Salida e Introducción a la POO (12 minutos)

**Descripción:** Aprende a reutilizar código mediante métodos y conoce las clases y objetos.

**Video:**  
[https://www.youtube.com/watch?v=_eVWnSaw4pM](https://www.youtube.com/watch?v=_eVWnSaw4pM)

**Conceptos clave:**
- Métodos.
- Parámetros.
- Scanner.
- Clases.
- Objetos.

**Práctica:**  
[https://onlinegdb.com/2OfABNkNw](https://onlinegdb.com/2OfABNkNw)


### Lección 5: Fundamentos de POO y Programa Sencillo (15 minutos)

**Descripción:** Construye una aplicación sencilla utilizando Programación Orientada a Objetos.

**Video:**  
[https://youtu.be/pJMyc8IQxPE?si=kKOcuITt07l17GnE](https://youtu.be/pJMyc8IQxPE?si=kKOcuITt07l17GnE)

**Conceptos clave:**
- Clases.
- Objetos.
- Atributos.
- Métodos.
- Encapsulación.
- Getters y Setters.

**Proyecto práctico:**  
[https://onlinegdb.com/PUg658Vqcq](https://onlinegdb.com/PUg658Vqcq)

### Lección 6: Consejos y Próximos Pasos (10 minutos)

**Descripción:** Buenas prácticas, errores comunes y recursos para continuar aprendiendo Java.

**Video:**  
https://youtu.be/[VIDEO-6]

**Conceptos clave:**
- Buenas prácticas.
- Errores frecuentes.
- Depuración básica.
- Recursos para seguir aprendiendo.

---

## Recursos Adicionales

### Código Fuente

Repositorio oficial:

https://github.com/GreeenMinds/java-fundamentals-course-GreenMinds/tree/main

### Actividades Prácticas

| Lección | Actividad | Enlace |
|----------|----------|----------|
| 1 | Hola Mundo | https://replit.com/[LECCION-1] |
| 2 | Variables y Tipos de Datos | https://onlinegdb.com/BGwNMV7ZHf |
| 3 | Estructuras de Control | https://replit.com/[LECCION-3] |
| 4 | Métodos y Clases | https://replit.com/[LECCION-4] |
| 5 | Proyecto POO | https://replit.com/[LECCION-5] |

---

## Elaboración

**Universidad Peruana de Ciencias Aplicadas (UPC)**

**Carrera:** Ingeniería de Software

**Curso:** Desarrollo de Aplicaciones Open Source

**Ciclo:** 202610

**NRC:** 11990

**Nombre del equipo:** GreenMinds

### Integrantes

- Aponte Pablo Isabel Luisa
- Astocondor Bazan Alejandra Isabel
- Diaz Martinez Alexther Kamil
- Dulanto Espino Leo César
- Laura Acosta Victor Jhosef

**Fecha de entrega:** [FECHA]

¡Gracias por completar el curso Fundamentos de Java para Principiantes!
